...
def assess(evaluation: Evaluation, problem: CompiledProblem) -> Admissibility:
    if not problem.avoiders:
        return Admissibility(feasible=True, max_excess=0.0)
    ...
    return Admissibility(feasible=maximum <= 1.0e-12, max_excess=maximum)


def preference_key(evaluation: Evaluation) -> tuple[int, float, float]:
...
def assert_evaluation_status(evaluation: Evaluation, problem: CompiledProblem) -> None:
    ...
    if evaluation.constraint_feasible != status.feasible or not math.isclose(
        evaluation.max_avoidance_excess, status.max_excess, abs_tol=1.0e-12
    ):
