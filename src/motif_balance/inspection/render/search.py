...
def render_search_svg(inspection: ResultInspection) -> bytes | None:
    ...
    parts.extend(
        [
            ...
            text(left - 10, top + plot_height + 4, "0", size=12, anchor="end", fill=MUTED),
            text(left - 10, top + 4, f"{y_max:.6g}", size=12, anchor="end", fill=MUTED),
            text(left, top + plot_height + 20, "0", size=12, anchor="middle", fill=MUTED),
            text(
                left + plot_width,
                top + plot_height + 20,
                x_max,
