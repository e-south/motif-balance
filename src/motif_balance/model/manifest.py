...
class RunManifest(FrozenModel):
    schema_version: Literal["run-manifest/v7"] = "run-manifest/v7"
    package_version: str
    ...
    def validate_evaluation_counts(self) -> Self:
        ...
        if self.search_diagnostics.schema_version != "search-diagnostics/v4":
            raise ValueError("run-manifest/v7 requires search-diagnostics/v4")
        ...
        if (
            ...
            raise ValueError("run-manifest/v7 requires exact, operation, and elite metadata")
        ...
        if elite_sequences != tuple(
            item.sequence
            for item in sorted(self.elites, key=lambda item: (-item.balance_score, item.sequence))
        ):
